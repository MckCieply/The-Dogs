...
$QUOTA_SLEEP_SEC    = 3600                             # 1 hour sleep on Pro quota hit (retry each hour until window resets)
$LOCK_STALE_SEC     = 3600                             # stale lock (>1h) is force-removed
...
function Complete-FeatureMerge {
   ...
   Write-Log 'INFO' "Fetching latest dev to prevent PR conflicts..."
    ...
    # 1. Attempt a standard merge with dev
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        # Give Claude a highly specific, scoped task to fix the merge state
        $conflictPrompt = @"
...
"@
    ...
    # 3. Automated Formatting Pass
    Write-Log 'INFO' "Running spotless:apply to enforce backend formatting rules..."
